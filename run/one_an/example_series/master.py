...
for i in range(0, SL):
    ...
    # in_PRE
    in_pre_path = os.path.join(path, 'in_PRE.ONEAN')
    copyfile('templates/in_PRE.ONEAN',in_pre_path)
    
    # in_EQUIL
    in_equil_path = os.path.join(path, 'in_EQUIL.ONEAN')
    copyfile('templates/in_EQUIL.ONEAN',in_equil_path)
    
    # in_RUN
    in_run_path = os.path.join(path, 'in_RUN.ONEAN')
    copyfile('templates/in_RUN.ONEAN',in_run_path)
    
    # in_LOG_RUN
    in_log_path = os.path.join(path, 'in_LOG_RUN.ONEAN')
    copyfile('templates/in_LOG_RUN.ONEAN',in_log_path)
