...
from parameters import N, M, n_t;
...
for i in range(0,n_t):
    ...
    # Positions (folded)
    temp_df = parse_LAMMPs_out_int("r.out", M*N+1, 9, names_in, i)
    r_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    rc_int[:,i*3:i*3+3] = temp_df[M*N:M*N+1,:];
    
    # Positions (unfolded)
    temp_df = parse_LAMMPs_out_int("ru.out", M*N+1, 9, names_in, i)
    ru_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    ruc_int[:,i*3:i*3+3] = temp_df[M*N:M*N+1,:];
    ...
    '''
    # Dipoles
    temp_df = parse_LAMMPs_out_int("mu.out", M*N+1, 9, names_in, i)
...
np.savez_compressed('run_data_r_lammps_ELEC', a=r_int, b=rc_int);
np.savez_compressed('run_data_ru_lammps_ELEC', a=ru_int, b=ruc_int);
