...
from parameters import N, M, n_tlog, numloop;
...
ruc_full = np.zeros((numloop*1,3*n_t));

# Folded positions
r_full = np.zeros((numloop*N*M,3*n_t));
rc_full = np.zeros((numloop*1,3*n_t));
...
for i in range(0,numloop):
    ...
    for j in range(0,n_t):
        ...
        # Positions (folded)
        temp_df = parse_LAMMPs_out_int('r.{:d}.out'.format(i+1), M*N+1, 9, names_in, j)
        r_int[:,j*3:j*3+3] = temp_df[0:M*N,:];
        rc_int[:,j*3:j*3+3] = temp_df[M*N:M*N+1,:];
        
        # Positions (unfolded)
        temp_df = parse_LAMMPs_out_int('ru.{:d}.out'.format(i+1), M*N+2*1, 9, names_in, j)
        ru_int[:,j*3:j*3+3] = temp_df[0:M*N,:];
        ruc_int[:,j*3:j*3+3] = temp_df[M*N:M*N+1,:];
        
        # Dipoles
        temp_df = parse_LAMMPs_out_int("mu.{:d}.out".format(i+1), M*N+1, 9, names_in, j)
...
np.savez_compressed('r_ELEC_LOG', a=r_full, b=rc_full);  
np.savez_compressed('ru_ELEC_LOG', a=ru_full, b=ruc_full);
