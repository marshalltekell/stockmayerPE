...
from functions import write2lammpsin_onecat_angle_sphere
...
rc = np.array([L/2, L/2, L/2], dtype=np.float);

#%% Write initial atom positions to in file for lammps

write2lammpsin_onecat_angle_sphere(N, M, L, "ONECAT_PRE.in", m, p, d, r, rc, mu)

#%% Run lammps serial with soft pairwise potential to move off overlaps

lmp = lammps();
lmp.file("in_PRE.ONECAT");

data = lmp.gather_atoms("x",1,3);
r = np.array(data).reshape(-1,3);
rc = r[M*N:M*N+1,:];
r = r[0:M*N,:];
rc = np.array([rc[0,0], rc[0,1],rc[0,2]])

#%% Write atom positions to in file for lammps after NPT

write2lammpsin_onecat_angle_sphere(N, M, L, "ONECAT_EQUIL.in", m, p, d, r, rc, mu)
