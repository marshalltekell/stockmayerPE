...
rho = 1.50*((I*m[1])/(I*m[1]+N*M*m[0])) + 1.06*((N*M*m[0])/(I*m[1]+N*M*m[0])); # g cm3
V = ((10**24)/(rho*N_Av))*(I*m[1] + N*M*m[0]); # Angstrom3
L = V**(1/3); # Angstrom
rccoul = 11.00;
...
for i in range(0,SL):
    ...
    # in_PRE
    in_pre_path = os.path.join(path, 'in_PRE.ONECAT')
    copyfile('templates/in_PRE.ONECAT',in_pre_path)
    
    # in_EQUIL
    in_equil_path = os.path.join(path, 'in_EQUIL.ONECAT')
    copyfile('templates/in_EQUIL.ONECAT',in_equil_path)
    
    # in_RUN
    in_run_path = os.path.join(path, 'in_RUN.ONECAT')
    copyfile('templates/in_RUN.ONECAT',in_run_path)
    
    # in_LOG_RUN
    in_log_path = os.path.join(path, 'in_LOG_RUN.ONECAT')
    copyfile('templates/in_LOG_RUN.ONECAT',in_log_path)
    ...
    sed(['-i', '',  's/!!!r22/{:.4f}/'.format(oij[2]), in_equil_path])
    sed(['-i', '',  's/!!!rc11/{:.4f}/'.format(rc[0]), in_equil_path])
    sed(['-i', '',  's/!!!rc12/{:.4f}/'.format(rc[1]), in_equil_path])
    sed(['-i', '',  's/!!!rc22/{:.4f}/'.format(rc[2]), in_equil_path])
    ...
    sed(['-i', '',  's/!!!r22/{:.4f}/'.format(oij[2]), in_run_path])
    sed(['-i', '',  's/!!!rc11/{:.4f}/'.format(rc[0]), in_run_path])
    sed(['-i', '',  's/!!!rc12/{:.4f}/'.format(rc[1]), in_run_path])
    sed(['-i', '',  's/!!!rc22/{:.4f}/'.format(rc[2]), in_run_path])
    ...
    sed(['-i', '',  's/!!!r22/{:.4f}/'.format(oij[2]), in_log_path])
    sed(['-i', '',  's/!!!rc11/{:.4f}/'.format(rc[0]), in_log_path])
    sed(['-i', '',  's/!!!rc12/{:.4f}/'.format(rc[1]), in_log_path])
    sed(['-i', '',  's/!!!rc22/{:.4f}/'.format(rc[2]), in_log_path])
    ...
    sed(['-i', '',  's/!!!r22/{:.4f}/'.format(oij[2]), param_path])
    sed(['-i', '',  's/!!!rc11/{:.4f}/'.format(rc[0]), param_path])
    sed(['-i', '',  's/!!!rc12/{:.4f}/'.format(rc[1]), param_path])
    sed(['-i', '',  's/!!!rc22/{:.4f}/'.format(rc[2]), param_path])
