...
for i in range(0,n_t):
    ...
    # Positions (folded)
    temp_df = parse_LAMMPs_out_int("r.out", M*N, 9, names_in, i)
    r_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    
    # Positions (unfolded)
    temp_df = parse_LAMMPs_out_int("ru.out", M*N, 9, names_in, i)
    ru_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    
    '''
    # Velocities
    temp_df = parse_LAMMPs_out_int("v.out", M*N, 9, names_in, i)
    v_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    
    # Forces
    temp_df = parse_LAMMPs_out_int("f.out", M*N, 9, names_in, i)
    f_int[:,i*3:i*3+3] = temp_df[0:M*N,:];
    '''
    # Dipoles
    temp_df = parse_LAMMPs_out_int("mu.out", M*N, 9, names_in, i)
...
np.savez_compressed('run_data_r_lammps_ELEC', a=r_int);
np.savez_compressed('run_data_ru_lammps_ELEC', a=ru_int);
