...
for i in range(0,numloop):
    ...
    for j in range(0,n_t):
        ...
        # Positions (folded)
        temp_df = parse_LAMMPs_out_int('r.{:d}.out'.format(i+1), M*N, 9, names_in, j)
        r_int[:,j*3:j*3+3] = temp_df[0:M*N,:];
        
        # Positions (unfolded)
        temp_df = parse_LAMMPs_out_int('ru.{:d}.out'.format(i+1), M*N, 9, names_in, j)
        ru_int[:,j*3:j*3+3] = temp_df[0:M*N,:];
        
        # Dipoles
        temp_df = parse_LAMMPs_out_int("mu.{:d}.out".format(i+1), M*N, 9, names_in, j)
...
np.savez_compressed('r_ELEC_LOG', a=r_full);  
np.savez_compressed('ru_ELEC_LOG', a=ru_full);
